...
if (isset($_GET['v'])) {
	switch ($_GET['v']) {
		...
		case 'wh':
			...
			break;
		case 'whdel':
			$webhook = 'https://api.telegram.org/bot'.$token.'/'.'deleteWebhook?url='.'https://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'];
			echo('Удаление вебхука'.$webhook);
			print_r(file_get_contents($webhook));
			break;
		case 'db':
			$query = "SET NAMES utf8;
...
CREATE TABLE `users` (
  ...
  PRIMARY KEY (`id`)
		...
		case 'update':
			...
			break;
		default:
		echo 'db - users - stats - drall - update - whtest - wh - start - pif - whdel';
			break;
	}
}
/*service*/
...
function request($method)
{
	...
	for ($i = 0; $i < 3; $i++) {
		try {
			...
			if ($code != 200) {
				curl_close($ch);
				feedbackAdmin('Error request curl code '.$code.' url '.$url.' result '.$result);
				die;
			}
			$result = json_decode($result, true);
			if (isset($result['ok']) and $result['ok'])
